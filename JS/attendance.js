...
function initialize(data){
    ...
    addMostLeastData(mostLeastData[1], '#mostA');
    addMostLeastData(mostLeastData[0], '#leastA');
    addCopyrightYear();
}
...
//------------CALCULATE AVERAGE PERCENTAGE OF MISSED VOTES---------------
//2 steps: 
...
//Takes the sorted list of statistics and checks if the 'last' person included in 10% hassame percentage as next person in the list, if so that member's data is pushed to an existing 'arr' array (leastEngaged or mostEngaged) used to populate the Most and Least tables.
...
function addMostLeastData(data, tableId) {
    ...
    data.forEach(function(line){
        table.append(
            $('<tr>')
            .append($('<td>').text(line.name))
            .append($('<td>').text(line.missed))
            .append($('<td>').text(line.missed_pct+'%'))
        );
    });
};
...
/*function addMostLeastData(mostLeastData) {
    ...
    var leastTable = $('#leastA');
    least.forEach(function(line){
        leastTable.append(
            $('<tr>')
            .append($('<td>').text(line.name))
            .append($('<td>').text(line.missed))
